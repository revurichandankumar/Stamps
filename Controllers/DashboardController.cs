...
namespace OneposStamps.Controllers
{
    public class DashboardController : BaseController
    {
        ...
        /// <returns></returns>
        public ActionResult Index(string domainId = null, string storeId = null, int reportid = 0, string type = null)
        {
            TempData["Type"] = type;
            if (!string.IsNullOrWhiteSpace(domainId) && !string.IsNullOrWhiteSpace(storeId))
            {
                db.Dispose();
                DataTable dbconnection = GetDatabaseConnectionValues(domainId);
                if (reportid == 20 || reportid == 21)
                {
                    ...
                    //db = Helper.SqlDBConnection.GetInstance(serverName: Convert.ToString(dbconnection.Rows[0]["Address"]), dbName: Convert.ToString(dbconnection.Rows[0]["DatabaseName"]) + "_Archive", userName: Convert.ToString(dbconnection.Rows[0]["UserName"]), password: Convert.ToString(dbconnection.Rows[0]["Password"]));
                }
                else
                {
                    db = Helper.SqlDBConnection.GetInstance(serverName: Convert.ToString(dbconnection.Rows[0]["Address"]), dbName: Convert.ToString(dbconnection.Rows[0]["DatabaseName"]), userName: Convert.ToString(dbconnection.Rows[0]["UserName"]), password: Convert.ToString(dbconnection.Rows[0]["Password"]));
